...
def encode_data(model, data_loader, log_step=10, logging=logger.info, backbone=False):
    ...
    max_n_word = 0
    for i, (images, img_lengths, captions, lengths, _) in enumerate(data_loader):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        if not backbone:
            img_emb, cap_emb, cap_len = model.forward_emb(images, captions, lengths, image_lengths=image_lengths)
        else:
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, save_path=None, cxc=False):
    ...
    with torch.no_grad():
        if opt.precomp_enc_type == 'basic':
            ...
            img_embs, cap_embs, cap_lens = encode_data(model, data_loader, backbone=True)
    ...
    if cxc:
        ...
        if not fold5:
            ...
            sims = shard_attn_scores(model, img_embs, cap_embs, cap_lens, opt, 1000)
